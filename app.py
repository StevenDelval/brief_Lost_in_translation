...
with tab3:
    df_dep= pd.DataFrame(df_join[['code_departement','type_objet',"date",'departement']].groupby(['code_departement']).count().reset_index())
    ...
    folium.Choropleth(
        ...
        data=df_dep,
        columns=["code_departement","type_objet"],
        key_on="feature.properties.code",
        fill_color="YlGn",
        fill_opacity=0.7,
        line_opacity=.1,
        legend_name="Type d'objet (%)",
    ).add_to(france)
